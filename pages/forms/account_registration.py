...
class AccountRegistrationForm(forms.Form):
    ...
    document_type = forms.ChoiceField(
        label="Tipo de documento", choices=Profile.DOCUMENTS_TYPE, required=False
    )
    document_number = forms.CharField(
        label="Número de documento", max_length=100, required=False
    )
    birth_date = forms.DateField(
        label="Fecha de nacimiento", widget=CustomDateInputWidget(), required=False
    )
    ...
    institution_name = forms.CharField(
        label="Universidad/Institución", max_length=100, required=False
    )
    study_program = forms.CharField(label="Carrera", max_length=100, required=False)
    student_id = forms.CharField(
        label="Identificación Estudiantil", max_length=50, required=False
    )
